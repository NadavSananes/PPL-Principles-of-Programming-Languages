...
export const parseCondExp = (param: Sexp[]) : Result<CondExp> => {
    let condclauses = mapResult(pasreCondClause, param);
    if(isOk(condclauses)){
        if(lastIsElse(condclauses.value)){
            if(!firstIsElse(condclauses.value)){
                return makeOk(makeCondExp(condclauses.value)); 
            }
            else{
                return makeFailure("Error! first cond-clause is else-clause.")
            }

        }
        else{
            return makeFailure("Error! missing else-clause");
        }
    }
    else{
        return condclauses;
...
export const pasreCondClause = (param: Sexp) : Result<CondClause> => {
    if(isArray(param)){
        let CExpList = mapResult(parseL31CExp, param);
        if(isOk(CExpList)){
            let firstCExp = mapv(CExpList, first);
            let restCExp = mapv(CExpList, rest);     
            if(isOk(firstCExp) && isOk(restCExp)){
                return makeOk(makeCondClause(firstCExp.value, restCExp.value));
            }
        }
        else{
            return CExpList;
...
const lastIsElse = (rest: CondClause[]): boolean => {
    if(!isEmpty(rest)){
        let elseClause = rest[rest.length - 1];
        if(isVarRef(elseClause.test)){
            return elseClause.test.var === 'else';
        }
    }
    return false;
}
const firstIsElse = (rest: CondClause[]): boolean => {
    if(!isEmpty(rest)){
        let elseClause = rest[0];
        if(isVarRef(elseClause.test)){
            return elseClause.test.var === 'else';
        }
    }
...
// CompoundCExp -> IfExp | ProcExp | LetExp | LitExp | AppExp | CondExp
...
const unparseCondExp = (ce : CondExp) : string =>
    `${map((x) => unparseCondClause(x), ce.condclauses).join(" ")}`;

export const unparseCondClause = (cc: CondClause) : string =>
    `(${unparseL31(cc.test)} ${map((x: CExp) => unparseL31(x), cc.then).join(" ")})`;
...
export const unparseL31 = (exp: Program | Exp): string =>
    ...
    isCondExp(exp) ? `(cond ${unparseCondExp(exp)})`    :  
    exp;
