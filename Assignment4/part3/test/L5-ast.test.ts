...
//  [union [T1 -> T1] [Empty -> T1]]
    ...
    it('parseTExp parses embedded union expressions', () => {
        expect(parseTE("(union (union number boolean) string)")).toSatisfy(isOkT(isUnionTExp));
    });

    it('parseTExp parses union types in proc argument position', () => {
        expect(parseTE("((union boolean number) -> (union string boolean))")).toSatisfy(isOkT(isProcTExp));
    });

    it('parseTExp parses union types in return type argument position', () => {
        expect(parseTE("(empty -> (union boolean number))")).toSatisfy(isOkT(isProcTExp));
    });

    it('parseTExp fails to parse union of bad type expressions', () => {
        expect(parseTE("(union string)")).toSatisfy(isFailure);
    });
...
describe('L5 parse with unions', () => {
    ...
    it('unparses union of atomic types in different positions: define, let, param, return types', () => {
        const dt1 = `
        (L5 
            (define (f : ((union boolean number) -> string)) (lambda ((x : (union boolean number))) : string "F OK"))
        )
        `;
        testProgram(dt1);
    });
    ...
    it('parses nested union expressions', () => {
        const dt2 = `
        (L5
            (define (f : (union number (union boolean string)) ("PPL")) 
        )
        `;
        testProgram(dt2);
    });
...
describe('L5 Unparse', () => {
    ...
    it('unparses union, nested unions in different positions', () => {
        const union = "(union a : number (union b : boolean c : string))";
