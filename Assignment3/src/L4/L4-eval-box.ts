...
import { applyEnv, applyEnvBdg, globalEnvAddBinding, makeExtEnv, setFBinding,
            theGlobalEnv, Env, FBinding, applyFrame, isFrame } from "./L4-env-box";
...
const evalBound = (exp: BoundExp, env: Env): Result<boolean> =>
    // option 1, just check if the varRef exist in the last frame.
    // isFrame(env.frame) ? isOk(applyFrame(env.frame, exp.var.var)) ? makeOk(true):
    //                     makeOk(false):
    // isOk(applyFrame(unbox(env.frame), exp.var.var)) ? makeOk(true):
    // makeOk(false);

    // option two, check if the varRef exist in the all env.
...
const evalTime = (exp: TimeExp, env: Env): Result<CompoundSExp> => {
    ...
    const evalutaeCExp = applicativeEval(exp.exp, env);
    const endTime = Date.now();
    if(isOk(evalutaeCExp)){
        return makeOk(makeCompoundSExp(evalutaeCExp.value, endTime - startTime));
    }
    else{
        return evalutaeCExp;
    }
