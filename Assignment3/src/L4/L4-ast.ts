...
export type CompoundExp = AppExp | IfExp | ProcExp | LetExp | LitExp | LetrecExp | SetExp | BoundExp | TimeExp;
...
// HW3: This is to help you get started.
export type BoundExp = {tag: "BoundExp"; var: VarRef}
export type TimeExp = {tag: "TimeExp"; exp: CExp}

// To help parser - define a type for reserved key words.
export type SpecialFormKeyword = "lambda" | "let" | "letrec" | "if" | "set!" | "quote" | "bound?" | "time";
const isSpecialFormKeyword = (x: string): x is SpecialFormKeyword =>
    ["if", "lambda", "let", "quote", "letrec", "set!", "bound?", "time"].includes(x);
...
// Q2.1
export const makeBoundExp = (v: VarRef) : BoundExp =>
    ({tag: "BoundExp", var: v});
// Q2.2
export const makeTimeExp = (e: CExp): TimeExp =>
    ({tag: "TimeExp", exp: e});
...
// Q2.1
export const isBoundExp = (x: any): x is BoundExp => x.tag === "BoundExp";
// Q2.2
export const isTimeExp = (x: any): x is TimeExp => x.tag === "TimeExp";
...
export const isCompoundExp = (x: any): x is CompoundExp =>
    ...
    isLetrecExp(x) || isSetExp(x) || isBoundExp(x) || isTimeExp(x);
...
export const parseL4SpecialForm = (op: SpecialFormKeyword, params: Sexp[]): Result<CExp> =>
    isNonEmptyList<Sexp>(params) ?
        ...
        op === "bound?" ? parseBoundExp(params):
        op === "time" ? parseTimeExp(params):
...
// Q2.1
const parseBoundExp = (params: Sexp[]): Result<BoundExp> =>
    params.length !== 1 ? makeFailure("More or less than one parameters"):
    !isString(params[0]) ? makeFailure("Problem with varRef"):
    makeOk(makeBoundExp(makeVarRef(params[0])));

// Q2.2
const parseTimeExp = (params: Sexp[]): Result<TimeExp> => {
    if(params.length !== 1)
        return makeFailure("More or less than one parameters");
    else{
        const timeCExp = parseL4CExp(params[0]);

        if(isOk(timeCExp)){
            if(!isCExp(timeCExp.value))
                return makeFailure("Not CExp!");

            return makeOk(makeTimeExp(timeCExp.value));
        }
        else{
            return timeCExp;
        }
...
export const unparse = (exp: Parsed): string =>
    ...
    isBoundExp(exp) ? `(bound? ${unparse(exp.var)})`:
    isTimeExp(exp) ? `(time ${unparse(exp.exp)})`:
    exp;
