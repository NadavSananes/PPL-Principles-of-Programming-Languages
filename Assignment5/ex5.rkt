...
(define equal-trees$ 
  (lambda (tree1 tree2 succ fail)
    (cond ((and (leaf? tree1) (leaf? tree2)) (succ (cons tree1 tree2)))
          ((and (empty-tree? tree1) (empty-tree? tree2)) (succ tree1))
          ((and (composite-tree? tree1) (composite-tree? tree2)) 
                      (equal-trees$ (first-subtree tree1) (first-subtree tree2)
                        (lambda (first-res) (equal-trees$ (rest-subtree tree1) (rest-subtree tree2)
                          (lambda (rest-res) (succ (add-subtree first-res rest-res))) fail)) fail))
...
(define diagn
  (lambda (lzl n)
    (cons-lzl (nth (nth lzl n) n) (lambda () (diagn lzl (+ n 1))))
  )
)
